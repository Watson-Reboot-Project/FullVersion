...
function populateSpans(){
		...
		populateFigureSpans(figSpans);						// pass spans to populateFigureSpans() to be populated with appropriate text
		...
		populateContentSpans(chapSpans);							// pass spans to populateContentSpans() to be populated with appropriate text
...
function populateContentSpans(spans) { 
	for (var i = 0; i < spans.length; i++) {
		var id = spans[i].className.replace("contentNum ", "");
		var content = getContentByID(id);
		
		if (spans[i].parentNode.className.indexOf("Section") >= 0) {
			// do nothing; we want to include the section number AND title
		}
		else {
...
function buildString(depth, name, curSectionID, targetSectionID) {
	...
	if (curSectionID == targetSectionID) {
		return textArr[depth] + " " + name;
		//else return textArr[depth] + ": " + name;
	}
